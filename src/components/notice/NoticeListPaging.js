...
function NoticeListPaging(props) {
  const [pageStart, setPageStart] = useState(1);
  const [totalPages, setTotalPages] = useState(null);
  const [currentPage, setCurrentPage] = useState(0);

  function onClickHandler(pageNum) {
    fetchFn("GET", `http://localhost:9229/notice/all?pageNum=${pageNum - 1}`).then(
      (data) => {
  ...
  function getPageNumInfo() {
    fetchFn("GET", `http://localhost:9229/notice/all?pageNum=0`).then((data) => {
        console.log(data)
        console.log(data.totalPages)
...
export default NoticeListPaging;
